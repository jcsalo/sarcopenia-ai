...
## New version uses Accession # in file name AND switches Patient ID and Accession #
...
## ... This requires a running Docker instance of sarcopenia-ai
## This script uses the saropenia-ai Imperial College Lodon script to identify L3
...
## ... which is contained within a "day" folder eg "DICOM 0720"
## ... which is contained within a folder eg "DICOM Stacks"
## Final L3 images are stored in L3 folder
## temp folder is used for Nifti files

def choose_dicom_directory():   # User Dialog to choose location of DICOM_XXX folder
    root = tk.Tk()
    root.withdraw()
    dicom_path = filedialog.askdirectory(title='Choose Location of enclosing DICOM_XXX folder to read')
    return dicom_path

def choose_output_directory():  # User dialog to choose location of L3 (results) folder
    root = tk.Tk()
    root.withdraw()
    output_directory = filedialog.askdirectory(title='Choose Location of for L3 DICOM and (temporary) NiftI folders')
...
## Determine whether there is a folder "L3_folder" inside of the dicom_folder
l3_folder_path = os.path.join(dicom_folder, "L3")
if not os.path.exists(l3_folder_path):
    os.makedirs(l3_folder_path)

## Determine whether there is a folder "temp" inside of the dicom_folder
nifti_folder_path = os.path.join(dicom_folder, "temp")
...
def generate_filename():
    ...
    filename = f"dicom_stacks_metadata_{formatted_datetime}.csv"
    return filename

def generate_errorfile():
    current_datetime = datetime.datetime.now()
    formatted_datetime = current_datetime.strftime("%Y-%m-%d")
    efilename = f"dicom_stacks_errors_{formatted_datetime}.csv"
    return efilename

## Create dicom_metadata .CSV file and write headers
dicom_metadata_filename = generate_filename()
dicom_metadata_path = os.path.join(output_folder,'L3',dicom_metadata_filename)
